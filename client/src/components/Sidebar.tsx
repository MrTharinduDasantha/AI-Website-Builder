...
const Sidebar = ({
  ...
  }, [project.conversation.length, isGenerating, isRevisionsLoading]);
  return (
    <div
        ...
        <form onSubmit={handleRevision} className="m-3 relative">
          <div className="flex items-center gap-2">
            <textarea
              ...
              disabled={isGenerating || isRevisionsLoading}
            />
            <button
              disabled={isGenerating || isRevisionsLoading || !input.trim()}
              className="absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {isGenerating || isRevisionsLoading ? (
                <Loader className="size-7 p-1.5 animate-spin text-white" />
              ) : (
